...
function attachEndModalControls() {
  ...
  // Change Level button now just restarts from level 1
  elements.changeLevelButton?.addEventListener("click", () => {
    closeEndModal();
    clearSavedLevel();
    resetGameState();
    if (state.levels.length > 0) {
      applyLevel(state.levels[0].level);
    }
    openStartModal();
  });
}
...
function renderTokensToGrid(tokens) {
  ...
  slots.forEach((slot) => {
    ...
    wrapper.className = "flex aspect-square items-center justify-center";
    ...
    if (slot.type === "ion") {
      ...
      hole.className =
        "h-20 w-20 rounded-full bg-gradient-to-b from-slate-700 to-slate-800 shadow-inner";
...
function createIonButton(species) {
  ...
  button.className = `ion-button h-16 w-16 box-border inline-flex items-center justify-center rounded-full border-2 ${colorClass} text-center text-sm font-semibold shadow-md transition hover:-translate-y-0.5 hover:shadow-lg`;
...
function buildOptionsFromPairing(pairing, correctFormula, correctName) {
  ...
  const selectedFormulaMistakes = selectRandomMistakes(formulaMistakes, 3);
  ...
  const selectedNameMistakes = selectRandomMistakes(nameMistakes, 3);
...
function buildFormulaOptions(cation, anion, correctFormula) {
  ...
  if (options.size < 4) {
    ...
    for (let i = 0; i < limit && options.size < 4; i += 1) {
...
function populateModal(challenge) {
  ...
  elements.remediationSubtitle.textContent = `${challenge.difficulty} • Pick the correct option in each column.`;
...
function renderOptions(container, options, type) {
  ...
  options.forEach((option) => {
    ...
    btn.className =
      "option-button w-full rounded-lg border border-slate-600 bg-slate-700/50 px-4 py-3 text-left text-sm transition hover:border-sky-300 hover:bg-slate-700";
...
function createHole() {
  const hole = document.createElement("div");
  hole.className = "h-16 w-16 box-border rounded-full bg-gradient-to-b from-slate-700 to-slate-800 shadow-inner border-2 border-transparent";
  return hole;
}
